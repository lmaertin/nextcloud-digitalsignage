...
- Instant message overlay width used 720px as an upper cap instead of a minimum, so it could shrink too narrow on smaller displays. Width now uses a 720px floor with the configured percentage as the effective size in between. Centering now uses a calculated `left` position instead of a horizontal `transform`, per feedback in #11.
...
- Documentation, asset cache versions and app metadata were updated for the `0.8.5` release.
